...
PROJECT_NAME ?= provider-dip
PROJECT_REPO ?= github.com/dip-software/$(PROJECT_NAME)
...
export TERRAFORM_NATIVE_PROVIDER_BINARY ?= terraform-provider-hsdp_v1.0.0_x5
...
GO_TEST_PARALLEL := $(shell echo $$(( $(NPROCS) / 2 < 1 ? 1 : $(NPROCS) / 2 )))
...
xpkg.build.provider-dip: do.build.images
